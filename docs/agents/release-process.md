...
```bash
bun add tailwindcss-flow-type@beta
```

Snapshot releases do not modify or commit version files. They use the pending changesets to calculate a version such as `1.0.1-beta-20260826130444`. Documentation, tooling, and other changes without a changeset do not publish a beta.
...
3. `release.yml` on `main` publishes the stable package to npm under the `latest` dist-tag and deploys the demo when publication succeeds.
