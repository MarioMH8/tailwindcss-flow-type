...
1. **Open an issue** using the appropriate template ([Bug Report](https://github.com/MarioMH8/tailwindcss-flow-type/issues/new?template=bug_report.yml) or [Feature Request](https://github.com/MarioMH8/tailwindcss-flow-type/issues/new?template=feature_request.yml))
...
|-----------------------------------------|------------------------------------------|---------------------------------------------------------------------------------|
| `.github/workflows/ci.yml`              | Push/PR to `main` or `develop`           | Runs lint, typecheck and tests via the reusable `check.yml`                     |
| `.github/workflows/check.yml`           | Reused by other workflows                | Installs dependencies and runs `lint:ci`, `typecheck`, `test`                   |
| `.github/workflows/release-prepare.yml` | Push to `develop`                        | Creates or updates the draft release PR to `main`                               |
| `.github/workflows/release.yml`         | Push to `main`                           | Publishes the stable package to npm under the `latest` tag and deploys the demo |
...
3. `release.yml` on `main` publishes the stable package to npm under the `latest` dist-tag and deploys the demo to GitHub Pages.
